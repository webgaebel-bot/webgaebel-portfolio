...
import { ArrowLeft, ArrowRight, CheckCircle2, ShieldCheck, Sparkles, Target, Workflow } from 'lucide-react';
...
const HeroNote = ({
  ...
  <div className="rounded-[22px] border border-white/12 bg-white/10 p-4 backdrop-blur-sm">
    <div className="text-xs font-semibold uppercase tracking-[0.2em] text-white/58">{label}</div>
    <div className="mt-2 text-sm leading-7 text-white/88">{text}</div>
  </div>
);
...
const DetailList = ({
  ...
  <ul className="space-y-3">
    {items.map((item) => (
      <li key={item} className={`flex items-start gap-3 ${variant === 'dark' ? 'text-white/82' : 'text-slate-600'}`}>
        <CheckCircle2 size={18} className={`mt-1 shrink-0 ${variant === 'dark' ? 'text-[var(--color-cyan)]' : 'text-[var(--color-teal)]'}`} />
          ...
          <div className="container relative mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid gap-10 lg:grid-cols-[1.04fr_0.96fr] lg:items-stretch">
              ...
              <motion.aside
                ...
                className="overflow-hidden rounded-[30px] border border-[rgba(255,255,255,0.12)] bg-[linear-gradient(145deg,var(--color-ink),var(--color-deep-navy),var(--color-corporate-blue))] text-white shadow-[0_32px_70px_rgba(11,61,102,0.2)]"
              >
                <div className="h-full p-6 sm:p-8">
                  <div className="rounded-[26px] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.12),rgba(255,255,255,0.03))] p-6">
                    ...
                    <div className="mt-6 grid gap-3 sm:grid-cols-2">
                      <HeroNote label="Structured for" text="Clear delivery scope, strong usability, and polished execution." />
                      <HeroNote label="Built around" text="Business goals, conversion paths, and reliable implementation." />
                    </div>
                  </div>
                  <div className="mt-5 grid gap-3">
                    <div className="rounded-[24px] border border-white/10 bg-[rgba(255,255,255,0.06)] p-5">
                      <div className="text-xs font-semibold uppercase tracking-[0.2em] text-white/58">Service snapshot</div>
                      <div className="mt-4 space-y-3 text-sm text-white/86">
                        <div className="flex items-center justify-between gap-4">
                          <span>Primary focus</span>
                          <span className="font-semibold text-white">{pageContent.primaryKeyword}</span>
                        </div>
                        <div className="flex items-center justify-between gap-4">
                          <span>Deliverables</span>
                          <span className="font-semibold text-white">{service.deliverables.length}</span>
                        </div>
                        <div className="flex items-center justify-between gap-4">
                          <span>Process stages</span>
                          <span className="font-semibold text-white">{pageContent.processSteps.length}</span>
                        </div>
                      </div>
                    </div>
                    <div className="rounded-[24px] border border-white/10 bg-white/8 p-5">
                      <div className="text-xs font-semibold uppercase tracking-[0.2em] text-white/58">Included in this service</div>
                      <div className="mt-4">
                        <DetailList items={service.points.slice(0, 3)} variant="dark" />
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div className="mt-14 grid gap-8">
              <motion.article
                ...
                className="theme-panel overflow-hidden p-0"
              >
                <div className="grid gap-0 lg:grid-cols-[0.92fr_1.08fr]">
                  ...
                  <div className="bg-[linear-gradient(145deg,var(--color-ink),var(--color-deep-navy),var(--color-corporate-blue))] p-6 text-white sm:p-8">
                    <div className="text-xs font-semibold uppercase tracking-[0.22em] text-white/58">Professional fit</div>
                    <h3 className="theme-heading mt-3 text-2xl font-bold">Designed for structured delivery</h3>
                    <p className="mt-4 leading-8 text-white/82">
                      This service page now presents scope, workflow, and delivery value in a cleaner professional layout for client review.
                    </p>
                    <div className="mt-6 grid gap-3 sm:grid-cols-2">
                      <HeroNote label="Best fit" text={service.outcomes} />
                      <HeroNote label="Service depth" text={`${service.deliverables.length} deliverables with a clear delivery path.`} />
                    </div>
                  </div>
                </div>
              </motion.article>
                ...
                >
                  ...
                  <div className="mt-6 grid gap-4">
                    {pageContent.processSteps.map((step, index) => (
                      <div
                        key={step.title}
                        className="grid gap-4 rounded-3xl border border-[rgba(11,61,102,0.08)] bg-[rgba(244,251,253,0.88)] p-5 md:grid-cols-[auto_1fr]"
                      >
                        <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-[linear-gradient(135deg,var(--color-corporate-blue),var(--color-teal))] text-sm font-bold text-white shadow-[0_14px_26px_rgba(11,61,102,0.16)]">
                          {index + 1}
                        </div>
                        <div>
              ...
              <div className="grid gap-8 xl:grid-cols-[0.95fr_1.05fr]">
                ...
                <motion.article
                  ...
                  className="overflow-hidden rounded-[28px] border border-[rgba(11,61,102,0.08)] bg-[linear-gradient(145deg,var(--color-ink),var(--color-deep-navy),var(--color-corporate-blue))] p-6 text-white shadow-[var(--shadow-card)] sm:p-8"
                >
                  <div className="text-xs font-semibold uppercase tracking-[0.22em] text-white/58">Why WebGaebel</div>
                  <h2 className="theme-heading mt-3 text-2xl font-bold text-white">Why Choose Us</h2>
                  <div className="mt-5 space-y-5 text-base leading-8 text-white/80 sm:text-lg">
                  ...
                  <div className="mt-6 grid gap-3 md:grid-cols-3">
                    {pageContent.whyChooseUsPoints.map((point) => (
                      <div
                        key={point}
                        className="rounded-2xl border border-white/12 bg-white/8 px-5 py-4 text-sm font-medium text-white/90"
                      >
                        {point}
                      </div>
                    ))}
        ...
        <div className="container relative mx-auto px-4 sm:px-6 lg:px-8">
            ...
            >
              <div className="h-full p-6 sm:p-8">
                <div className="rounded-[26px] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.12),rgba(255,255,255,0.03))] p-6">
                  ...
                  <div className="mt-6 grid gap-3 sm:grid-cols-2">
                    <HeroNote label="Includes" text="Planning, execution, and delivery-ready structure." />
                    <HeroNote label="Focus" text="Performance, clarity, and business-fit implementation." />
                  </div>
                </div>
                <div className="mt-5 rounded-[24px] border border-white/10 bg-[rgba(255,255,255,0.06)] p-5">
                  <div className="flex items-center gap-3">
                    <ShieldCheck className="h-5 w-5 text-[var(--color-cyan)]" />
                    <div className="text-sm font-semibold uppercase tracking-[0.2em] text-white/68">Professional fit</div>
                  </div>
                  <p className="mt-3 text-sm leading-7 text-white/84">
                    Designed for teams that want a clean scope, dependable delivery, and a more polished service presentation.
                  </p>
                </div>
              </div>
            </motion.aside>
          ...
          <div className="mt-14 grid gap-8 xl:grid-cols-[1.06fr_0.94fr]">
            ...
            >
              ...
              <div className="mt-5">
                <DetailList items={service.points} />
              </div>
            </motion.article>
            ...
            >
              ...
              <div className="mt-5">
                <DetailList items={service.process} />
              </div>
            </motion.article>
          </div>
          ...
          <div className="mt-8 grid gap-8 xl:grid-cols-[0.94fr_1.06fr]">
            ...
            >
              ...
              <div className="mt-5">
                <DetailList items={service.industries} />
              </div>
            </motion.article>
